...
void render_frame(float A, float B) {
    ...
    for (float t = 0; t < 2 * M_PI; t += t_spacing) {
        // Trefoil knot parametric equations
        float x = sin(t) + 2 * sin(2 * t);
        float y = cos(t) - 2 * cos(2 * t);
        float z = -sin(3 * t);

        // Rotate in 3D (Z then X)
        ...
        float ooz = 1.0f / (K2 + z_rot);
        int xp = (int)(screen_width / 2 + K1 * ooz * x_rot);
        int yp = (int)(screen_height / 2 - K1 * ooz * y_rot * 0.5f);

        float L = (cos(t) + sin(3 * t)) * 0.5f;

        if (xp >= 0 && xp < screen_width && yp >= 0 && yp < screen_height) {
            if (ooz > zbuffer[yp][xp]) {
                zbuffer[yp][xp] = ooz;
                int luminance_index = std::max(0, std::min(11, (int)(L * 8 + 6)));
